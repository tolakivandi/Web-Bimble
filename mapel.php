 <?php
    ...
    $mapel = mysqli_query($connection, "SELECT * FROM mapel");
                 ...
                 <?php foreach ($mapel as $item) : ?>
                     <tr class="bg-cyan-200 cursor-pointer duration-300">
                         ...
                         <td class="py-3 px-6 flex gap-5 justify-center">
                             <a href="./mapel/updatemapel.php?id=<?= $item["id_mapel"] ?>" class="px-4 py-3 bg-cyan-500 rounded-xl text-black font-semibold border-black border-2">Update</a> |
                             <a href="./mapel/deletemapel.php?id=<?= $item["id_mapel"] ?>" onclick="return confirm('yakin?');" class="px-4 py-3 bg-cyan-500 rounded-xl text-black font-semibold border-black border-2">Delete</a>
                         </td>
                     </tr>
                     <?php $i++; ?>
                 <?php endforeach; ?>
...
-webkit-box-shadow: 7px 14px 23px 0px rgba(0,0,0,0.75);
-moz-box-shadow: 7px 14px 23px 0px rgba(0,0,0,0.75);"> <a href="./mapel/createmapel.php">Tambah Data</a> </button>
     </div>
 </body>
